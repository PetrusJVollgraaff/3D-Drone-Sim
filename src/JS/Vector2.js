class Vector2 {
  ...
  static formOffsets(info) {
    return new Vector2({ x: info.offsetX, y: info.offsetY });
  }
  ...
  scale(scaler) {
    return new Vector2({ x: this.x * scaler, y: this.y * scaler });
  }

  min(v) {
    return new Vector2({ x: Math.min(this.x, v.x), y: Math.min(this.y, v.y) });
  }

  max(v) {
    return new Vector2({ x: Math.max(this.x, v.x), y: Math.max(this.y, v.y) });
  }
